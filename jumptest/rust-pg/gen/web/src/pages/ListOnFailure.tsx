...
// Row shape returned by GET /api/onfailure and /api/onfailure/view/{id}
// -- matches the OnFailureView class generated for the dotnet/rust servers.
export interface OnFailureView {
  id: number;
  action: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "OnFailure";
...
async function fetchList(api: ApiClient): Promise<OnFailureView[]> {
  return api.get<OnFailureView[]>("/api/onfailure");
}

export default function ListOnFailure() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<OnFailureView[] | null>(null);
  ...
  const listRef = useRef<OnFailureView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListOnFailure: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListOnFailure: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent<string>) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<OnFailureView>(`/api/onfailure/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListOnFailure: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-onfailure?returnUrl=${returnUrl}`);
  }

  function onEdit(item: OnFailureView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-onfailure/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>On Failure Code</h1>
      <p>This page demonstrates fetching On Failure Code data from the server.</p>
