...
// Raw shape sent/received by GET|POST|PUT /api/execlog[/{id}] -- matches
// the ExecLog domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "ExecLogHistory : ExecLog").
export type ExecLogHistory = ExecLog;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Execution ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "token", label: "Token", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "workflow_id", label: "Process", kind: "enum", fkVar: "workflow", isGlobal: false, isId: false },
  { key: "start_time", label: "Start Time", kind: "date", fkVar: "", isGlobal: false, isId: false },
  { key: "end_time", label: "End Time", kind: "date", fkVar: "", isGlobal: false, isId: false },
  { key: "exec_status_id", label: "Status", kind: "number", fkVar: "", isGlobal: false, isId: false },
  { key: "stdout", label: "Stdout", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "stderr", label: "Stderr", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyExecLog(): ExecLog {
  ...
  return obj as unknown as ExecLog;
}
...
export default function EditExecLog() {
  ...
  const [formData, setFormData] = useState<ExecLog>(() => createEmptyExecLog());
  const [historyList, setHistoryList] = useState<ExecLogHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyExecLog());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<ExecLog>(`/api/execlog/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditExecLog: error loading record", err);
      }

      try {
        const history = await api.get<ExecLogHistory[]>(`/api/execlog/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditExecLog: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditExecLog: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<ExecLog>("/api/execlog", formData);
      } else {
        await api.put<ExecLog>(`/api/execlog/${id}`, formData);
      ...
      console.error("EditExecLog: error saving execlog", err);
    }
  ...
  return (
    <>
      {id == null ? <h3>Create Execution Log</h3> : <h3>Edit Execution Log {rwkString}</h3>}
