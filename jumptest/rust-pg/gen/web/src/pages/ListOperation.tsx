...
// Row shape returned by GET /api/operation and /api/operation/view/{id}
// -- matches the OperationView class generated for the dotnet/rust servers.
...
const DOMAIN_OBJECT_NAME = "Operation";
...
async function fetchList(api: ApiClient): Promise<OperationView[]> {
  return api.get<OperationView[]>("/api/operation");
}
...
export default function ListOperation() {
  ...
  const [list, setList] = useState<OperationView[] | null>(null);
  ...
  const listRef = useRef<OperationView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListOperation: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListOperation: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<OperationView>(`/api/operation/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListOperation: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-operation?returnUrl=${returnUrl}`);
  }

  function onEdit(item: OperationView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-operation/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Operations</h1>
      <p>This page demonstrates fetching Operations data from the server.</p>
