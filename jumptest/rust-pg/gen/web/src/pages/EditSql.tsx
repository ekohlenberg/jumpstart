...
// Raw shape sent/received by GET|POST|PUT /api/sql[/{id}] -- matches
// the Sql domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "SqlHistory : Sql").
export type SqlHistory = Sql;
...
export default function EditSql() {
  ...
  const [formData, setFormData] = useState<Sql>(() => createEmptySql());
  const [historyList, setHistoryList] = useState<SqlHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptySql());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<Sql>(`/api/sql/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditSql: error loading record", err);
      }

      try {
        const history = await api.get<SqlHistory[]>(`/api/sql/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditSql: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditSql: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<Sql>("/api/sql", formData);
      } else {
        await api.put<Sql>(`/api/sql/${id}`, formData);
      ...
      console.error("EditSql: error saving sql", err);
    }
    navigate(returnUrl ?? "/sql");
  };
  ...
  // Soft delete -- DELETE /api/sql/{id} sets is_active=0 server-side
  // (see SqlLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/sql/${id}`);
    } catch (err) {
      console.error("EditSql: error deleting sql", err);
      return;
    }
    navigate(returnUrl ?? "/sql");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Query</h3> : <h3>Edit Query {rwkString}</h3>}
