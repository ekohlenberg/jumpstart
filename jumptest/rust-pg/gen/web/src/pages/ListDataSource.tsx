...
// Row shape returned by GET /api/datasource and /api/datasource/view/{id}
// -- matches the DataSourceView class generated for the dotnet/rust servers.
...
const DOMAIN_OBJECT_NAME = "DataSource";
...
async function fetchList(api: ApiClient): Promise<DataSourceView[]> {
  return api.get<DataSourceView[]>("/api/datasource");
}
...
export default function ListDataSource() {
  ...
  const [list, setList] = useState<DataSourceView[] | null>(null);
  ...
  const listRef = useRef<DataSourceView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListDataSource: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListDataSource: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<DataSourceView>(`/api/datasource/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListDataSource: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-datasource?returnUrl=${returnUrl}`);
  }

  function onEdit(item: DataSourceView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-datasource/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Data Source</h1>
      <p>This page demonstrates fetching Data Source data from the server.</p>
