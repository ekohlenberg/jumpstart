...
// Row shape returned by GET /api/crondom and /api/crondom/view/{id}
// -- matches the CronDomView class generated for the dotnet/rust servers.
export interface CronDomView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "CronDom";
...
async function fetchList(api: ApiClient): Promise<CronDomView[]> {
  return api.get<CronDomView[]>("/api/crondom");
}

export default function ListCronDom() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<CronDomView[] | null>(null);
  ...
  const listRef = useRef<CronDomView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListCronDom: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListCronDom: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent<string>) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<CronDomView>(`/api/crondom/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListCronDom: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-crondom?returnUrl=${returnUrl}`);
  }

  function onEdit(item: CronDomView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-crondom/${item.id}?returnUrl=${returnUrl}`);
  }
