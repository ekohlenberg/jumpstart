...
// Raw shape sent/received by GET|POST|PUT /api/workflowtype[/{id}] -- matches
// the WorkflowType domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "WorkflowTypeHistory : WorkflowType").
export type WorkflowTypeHistory = WorkflowType;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Workflow Type ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
...
function createEmptyWorkflowType(): WorkflowType {
  ...
  return obj as unknown as WorkflowType;
}
...
export default function EditWorkflowType() {
  ...
  const [formData, setFormData] = useState<WorkflowType>(() => createEmptyWorkflowType());
  const [historyList, setHistoryList] = useState<WorkflowTypeHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyWorkflowType());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<WorkflowType>(`/api/workflowtype/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditWorkflowType: error loading record", err);
      }

      try {
        const history = await api.get<WorkflowTypeHistory[]>(`/api/workflowtype/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditWorkflowType: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditWorkflowType: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<WorkflowType>("/api/workflowtype", formData);
      } else {
        await api.put<WorkflowType>(`/api/workflowtype/${id}`, formData);
      ...
      console.error("EditWorkflowType: error saving workflowtype", err);
    }
    navigate(returnUrl ?? "/workflowtype");
  };
  ...
  // Soft delete -- DELETE /api/workflowtype/{id} sets is_active=0 server-side
  // (see WorkflowTypeLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Workflow Type? This cannot be undone.`)) return;
    try {
      await api.del(`/api/workflowtype/${id}`);
    } catch (err) {
      console.error("EditWorkflowType: error deleting workflowtype", err);
      return;
    }
    navigate(returnUrl ?? "/workflowtype");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Workflow Type</h3> : <h3>Edit Workflow Type {rwkString}</h3>}
