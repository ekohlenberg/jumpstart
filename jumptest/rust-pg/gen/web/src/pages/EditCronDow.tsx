...
// Raw shape sent/received by GET|POST|PUT /api/crondow[/{id}] -- matches
// the CronDow domain class generated for the dotnet/rust servers (every
// attribute, unlike the *View interface the list page works with).
export interface CronDow {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
// shared/dotnet/domain/template.generated.cs.cshtml's "CronDowHistory : CronDow").
export type CronDowHistory = CronDow;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Day Of Week ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Day Of Week", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyCronDow(): CronDow {
  ...
  return obj as unknown as CronDow;
}
...
export default function EditCronDow() {
  ...
  const [formData, setFormData] = useState<CronDow>(() => createEmptyCronDow());
  const [historyList, setHistoryList] = useState<CronDowHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyCronDow());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<CronDow>(`/api/crondow/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditCronDow: error loading record", err);
      }

      try {
        const history = await api.get<CronDowHistory[]>(`/api/crondow/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditCronDow: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditCronDow: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<CronDow>("/api/crondow", formData);
      } else {
        await api.put<CronDow>(`/api/crondow/${id}`, formData);
      ...
      console.error("EditCronDow: error saving crondow", err);
    }
    navigate(returnUrl ?? "/crondow");
  };
  ...
  // Soft delete -- DELETE /api/crondow/{id} sets is_active=0 server-side
  // (see CronDowLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/crondow/${id}`);
    } catch (err) {
      console.error("EditCronDow: error deleting crondow", err);
      return;
    }
    navigate(returnUrl ?? "/crondow");
  }
