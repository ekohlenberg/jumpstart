...
// shared/dotnet/domain/template.generated.cs.cshtml's "OperationHistory : Operation").
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Action ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "objectname", label: "Object", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "methodname", label: "Method", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
export default function EditOperation() {
  ...
  const [formData, setFormData] = useState<Operation>(() => createEmptyOperation());
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyOperation());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<Operation>(`/api/operation/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditOperation: error loading record", err);
      }

      try {
        const history = await api.get<OperationHistory[]>(`/api/operation/${id}/history`);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditOperation: error loading enum options", err));
  ...
  // Soft delete -- DELETE /api/operation/{id} sets is_active=0 server-side
  // (see OperationLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Operations? This cannot be undone.`)) return;
    try {
      await api.del(`/api/operation/${id}`);
    } catch (err) {
      console.error("EditOperation: error deleting operation", err);
      return;
    }
    navigate(returnUrl ?? "/operation");
  }

  const rwkString = [formData.objectname, formData.methodname]
