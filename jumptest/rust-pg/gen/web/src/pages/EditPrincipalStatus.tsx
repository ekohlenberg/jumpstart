...
// Raw shape sent/received by GET|POST|PUT /api/principalstatus[/{id}] -- matches
// the PrincipalStatus domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "PrincipalStatusHistory : PrincipalStatus").
export type PrincipalStatusHistory = PrincipalStatus;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Principal Status ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
...
function createEmptyPrincipalStatus(): PrincipalStatus {
  ...
  return obj as unknown as PrincipalStatus;
}
...
export default function EditPrincipalStatus() {
  ...
  const [formData, setFormData] = useState<PrincipalStatus>(() => createEmptyPrincipalStatus());
  const [historyList, setHistoryList] = useState<PrincipalStatusHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyPrincipalStatus());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<PrincipalStatus>(`/api/principalstatus/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditPrincipalStatus: error loading record", err);
      }

      try {
        const history = await api.get<PrincipalStatusHistory[]>(`/api/principalstatus/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditPrincipalStatus: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditPrincipalStatus: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<PrincipalStatus>("/api/principalstatus", formData);
      } else {
        await api.put<PrincipalStatus>(`/api/principalstatus/${id}`, formData);
      ...
      console.error("EditPrincipalStatus: error saving principalstatus", err);
    }
    navigate(returnUrl ?? "/principalstatus");
  };
  ...
  // Soft delete -- DELETE /api/principalstatus/{id} sets is_active=0 server-side
  // (see PrincipalStatusLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/principalstatus/${id}`);
    } catch (err) {
      console.error("EditPrincipalStatus: error deleting principalstatus", err);
      return;
    }
    navigate(returnUrl ?? "/principalstatus");
  }
