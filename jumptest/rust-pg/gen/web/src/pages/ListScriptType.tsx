...
// Row shape returned by GET /api/scripttype and /api/scripttype/view/{id}
// -- matches the ScriptTypeView class generated for the dotnet/rust servers.
export interface ScriptTypeView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "ScriptType";
...
async function fetchList(api: ApiClient): Promise<ScriptTypeView[]> {
  return api.get<ScriptTypeView[]>("/api/scripttype");
}

export default function ListScriptType() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<ScriptTypeView[] | null>(null);
  ...
  const listRef = useRef<ScriptTypeView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListScriptType: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListScriptType: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<ScriptTypeView>(`/api/scripttype/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListScriptType: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-scripttype?returnUrl=${returnUrl}`);
  }

  function onEdit(item: ScriptTypeView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-scripttype/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Script Type</h1>
      <p>This page demonstrates fetching Script Type data from the server.</p>
