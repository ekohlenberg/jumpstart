...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Server Node Type ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
...
export default function EditServerNodeType() {
  ...
  // Soft delete -- DELETE /api/servernodetype/{id} sets is_active=0 server-side
  // (see ServerNodeTypeLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Server Node Type? This cannot be undone.`)) return;
    try {
      await api.del(`/api/servernodetype/${id}`);
    } catch (err) {
      console.error("EditServerNodeType: error deleting servernodetype", err);
      return;
    }
    navigate(returnUrl ?? "/servernodetype");
  }
