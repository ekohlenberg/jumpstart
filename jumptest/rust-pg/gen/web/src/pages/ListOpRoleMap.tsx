...
// Row shape returned by GET /api/oprolemap and /api/oprolemap/view/{id}
// -- matches the OpRoleMapView class generated for the dotnet/rust servers.
export interface OpRoleMapView {
  id: number;
  op_id: number;
  op_role_id: number;
  is_active: number;
  ...
  txn_id: number;
  op_objectname: string;
  op_methodname: string;
  op_role_name: string;
}

const DOMAIN_OBJECT_NAME = "OpRoleMap";
...
const columns: DataTableColumn[] = [
  ...
  { key: "op_objectname", label: "Operation ID Object" },
  { key: "op_methodname", label: "Operation ID Method" },
  { key: "op_role_name", label: "Role ID" },
];
...
async function fetchList(api: ApiClient): Promise<OpRoleMapView[]> {
  return api.get<OpRoleMapView[]>("/api/oprolemap");
}

export default function ListOpRoleMap() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<OpRoleMapView[] | null>(null);
  ...
  const listRef = useRef<OpRoleMapView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListOpRoleMap: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListOpRoleMap: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<OpRoleMapView>(`/api/oprolemap/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListOpRoleMap: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-oprolemap?returnUrl=${returnUrl}`);
  }

  function onEdit(item: OpRoleMapView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-oprolemap/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Operation Group Map</h1>
      <p>This page demonstrates fetching Operation Group Map data from the server.</p>
