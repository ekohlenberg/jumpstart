...
// Raw shape sent/received by GET|POST|PUT /api/schedule[/{id}] -- matches
// the Schedule domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "ScheduleHistory : Schedule").
export type ScheduleHistory = Schedule;
...
export default function EditSchedule() {
  ...
  const [formData, setFormData] = useState<Schedule>(() => createEmptySchedule());
  const [historyList, setHistoryList] = useState<ScheduleHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const record = await api.get<Schedule>(`/api/schedule/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditSchedule: error loading record", err);
      }

      try {
        const history = await api.get<ScheduleHistory[]>(`/api/schedule/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditSchedule: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditSchedule: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<Schedule>("/api/schedule", formData);
      } else {
        await api.put<Schedule>(`/api/schedule/${id}`, formData);
      ...
      console.error("EditSchedule: error saving schedule", err);
    }
  ...
  return (
    <>
      {id == null ? <h3>Create Schedule</h3> : <h3>Edit Schedule {rwkString}</h3>}
