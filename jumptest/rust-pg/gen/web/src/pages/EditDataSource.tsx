...
// Raw shape sent/received by GET|POST|PUT /api/datasource[/{id}] -- matches
// the DataSource domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "DataSourceHistory : DataSource").
export type DataSourceHistory = DataSource;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Data Source ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
...
function createEmptyDataSource(): DataSource {
  ...
  return obj as unknown as DataSource;
}
...
export default function EditDataSource() {
  ...
  const [formData, setFormData] = useState<DataSource>(() => createEmptyDataSource());
  const [historyList, setHistoryList] = useState<DataSourceHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyDataSource());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<DataSource>(`/api/datasource/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditDataSource: error loading record", err);
      }

      try {
        const history = await api.get<DataSourceHistory[]>(`/api/datasource/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditDataSource: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditDataSource: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<DataSource>("/api/datasource", formData);
      } else {
        await api.put<DataSource>(`/api/datasource/${id}`, formData);
      ...
      console.error("EditDataSource: error saving datasource", err);
    }
    navigate(returnUrl ?? "/datasource");
  };
  ...
  // Soft delete -- DELETE /api/datasource/{id} sets is_active=0 server-side
  // (see DataSourceLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/datasource/${id}`);
    } catch (err) {
      console.error("EditDataSource: error deleting datasource", err);
      return;
    }
    navigate(returnUrl ?? "/datasource");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Data Source</h3> : <h3>Edit Data Source {rwkString}</h3>}
