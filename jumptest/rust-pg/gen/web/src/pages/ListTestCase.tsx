...
// Row shape returned by GET /api/testcase and /api/testcase/view/{id}
// -- matches the TestCaseView class generated for the dotnet/rust servers.
...
const DOMAIN_OBJECT_NAME = "TestCase";
...
async function fetchList(api: ApiClient): Promise<TestCaseView[]> {
  return api.get<TestCaseView[]>("/api/testcase");
}
...
export default function ListTestCase() {
  ...
  const [list, setList] = useState<TestCaseView[] | null>(null);
  ...
  const listRef = useRef<TestCaseView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListTestCase: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListTestCase: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent<string>) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<TestCaseView>(`/api/testcase/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListTestCase: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-testcase?returnUrl=${returnUrl}`);
  }

  function onEdit(item: TestCaseView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-testcase/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Test Case</h1>
      <p>This page demonstrates fetching Test Case data from the server.</p>
