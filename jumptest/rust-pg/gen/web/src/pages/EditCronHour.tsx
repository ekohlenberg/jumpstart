...
// Raw shape sent/received by GET|POST|PUT /api/cronhour[/{id}] -- matches
// the CronHour domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "CronHourHistory : CronHour").
export type CronHourHistory = CronHour;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Hour ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Hour", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyCronHour(): CronHour {
  ...
  return obj as unknown as CronHour;
}
...
export default function EditCronHour() {
  ...
  const [formData, setFormData] = useState<CronHour>(() => createEmptyCronHour());
  const [historyList, setHistoryList] = useState<CronHourHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyCronHour());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<CronHour>(`/api/cronhour/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditCronHour: error loading record", err);
      }

      try {
        const history = await api.get<CronHourHistory[]>(`/api/cronhour/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditCronHour: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditCronHour: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<CronHour>("/api/cronhour", formData);
      } else {
        await api.put<CronHour>(`/api/cronhour/${id}`, formData);
      ...
      console.error("EditCronHour: error saving cronhour", err);
    }
    navigate(returnUrl ?? "/cronhour");
  };
  ...
  // Soft delete -- DELETE /api/cronhour/{id} sets is_active=0 server-side
  // (see CronHourLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/cronhour/${id}`);
    } catch (err) {
      console.error("EditCronHour: error deleting cronhour", err);
      return;
    }
    navigate(returnUrl ?? "/cronhour");
  }
