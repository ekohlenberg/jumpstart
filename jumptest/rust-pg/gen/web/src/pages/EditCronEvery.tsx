...
// Raw shape sent/received by GET|POST|PUT /api/cronevery[/{id}] -- matches
// the CronEvery domain class generated for the dotnet/rust servers (every
// attribute, unlike the *View interface the list page works with).
export interface CronEvery {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
// shared/dotnet/domain/template.generated.cs.cshtml's "CronEveryHistory : CronEvery").
export type CronEveryHistory = CronEvery;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Every ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Run At", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
const OWN_COLUMNS: DataTableColumn[] = [
  { key: "id", label: "Every ID" },
  { key: "name", label: "Run At" },
...
function createEmptyCronEvery(): CronEvery {
  ...
  return obj as unknown as CronEvery;
}
...
export default function EditCronEvery() {
  ...
  const [formData, setFormData] = useState<CronEvery>(() => createEmptyCronEvery());
  const [historyList, setHistoryList] = useState<CronEveryHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyCronEvery());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<CronEvery>(`/api/cronevery/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditCronEvery: error loading record", err);
      }

      try {
        const history = await api.get<CronEveryHistory[]>(`/api/cronevery/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditCronEvery: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditCronEvery: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<CronEvery>("/api/cronevery", formData);
      } else {
        await api.put<CronEvery>(`/api/cronevery/${id}`, formData);
      ...
      console.error("EditCronEvery: error saving cronevery", err);
    }
    navigate(returnUrl ?? "/cronevery");
  };
  ...
  // Soft delete -- DELETE /api/cronevery/{id} sets is_active=0 server-side
  // (see CronEveryLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/cronevery/${id}`);
    } catch (err) {
      console.error("EditCronEvery: error deleting cronevery", err);
      return;
    }
    navigate(returnUrl ?? "/cronevery");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create </h3> : <h3>Edit  {rwkString}</h3>}
