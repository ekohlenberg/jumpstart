...
// Raw shape sent/received by GET|POST|PUT /api/onfailure[/{id}] -- matches
// the OnFailure domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "OnFailureHistory : OnFailure").
export type OnFailureHistory = OnFailure;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "On Failure ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "action", label: "Action", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyOnFailure(): OnFailure {
  ...
  return obj as unknown as OnFailure;
}
...
export default function EditOnFailure() {
  ...
  const [formData, setFormData] = useState<OnFailure>(() => createEmptyOnFailure());
  const [historyList, setHistoryList] = useState<OnFailureHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyOnFailure());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<OnFailure>(`/api/onfailure/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditOnFailure: error loading record", err);
      }

      try {
        const history = await api.get<OnFailureHistory[]>(`/api/onfailure/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditOnFailure: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditOnFailure: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<OnFailure>("/api/onfailure", formData);
      } else {
        await api.put<OnFailure>(`/api/onfailure/${id}`, formData);
      ...
      console.error("EditOnFailure: error saving onfailure", err);
    }
    navigate(returnUrl ?? "/onfailure");
  };
  ...
  // Soft delete -- DELETE /api/onfailure/{id} sets is_active=0 server-side
  // (see OnFailureLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this On Failure Code? This cannot be undone.`)) return;
    try {
      await api.del(`/api/onfailure/${id}`);
    } catch (err) {
      console.error("EditOnFailure: error deleting onfailure", err);
      return;
    }
    navigate(returnUrl ?? "/onfailure");
  }

  const rwkString = [formData.action]
  ...
  return (
    <>
      {id == null ? <h3>Create On Failure Code</h3> : <h3>Edit On Failure Code {rwkString}</h3>}
