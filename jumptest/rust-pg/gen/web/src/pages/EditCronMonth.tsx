...
// Raw shape sent/received by GET|POST|PUT /api/cronmonth[/{id}] -- matches
// the CronMonth domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "CronMonthHistory : CronMonth").
export type CronMonthHistory = CronMonth;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Month ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Month", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyCronMonth(): CronMonth {
  ...
  return obj as unknown as CronMonth;
}
...
export default function EditCronMonth() {
  ...
  const [formData, setFormData] = useState<CronMonth>(() => createEmptyCronMonth());
  const [historyList, setHistoryList] = useState<CronMonthHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyCronMonth());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<CronMonth>(`/api/cronmonth/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditCronMonth: error loading record", err);
      }

      try {
        const history = await api.get<CronMonthHistory[]>(`/api/cronmonth/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditCronMonth: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditCronMonth: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<CronMonth>("/api/cronmonth", formData);
      } else {
        await api.put<CronMonth>(`/api/cronmonth/${id}`, formData);
      ...
      console.error("EditCronMonth: error saving cronmonth", err);
    }
    navigate(returnUrl ?? "/cronmonth");
  };
  ...
  // Soft delete -- DELETE /api/cronmonth/{id} sets is_active=0 server-side
  // (see CronMonthLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/cronmonth/${id}`);
    } catch (err) {
      console.error("EditCronMonth: error deleting cronmonth", err);
      return;
    }
    navigate(returnUrl ?? "/cronmonth");
  }
