...
// Raw shape sent/received by GET|POST|PUT /api/testresultstatus[/{id}] -- matches
// the TestResultStatus domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "TestResultStatusHistory : TestResultStatus").
export type TestResultStatusHistory = TestResultStatus;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Test Result Status ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
...
const OWN_COLUMNS: DataTableColumn[] = [
  { key: "id", label: "Test Result Status ID" },
  { key: "name", label: "Name" },
...
function createEmptyTestResultStatus(): TestResultStatus {
  ...
  return obj as unknown as TestResultStatus;
}
...
export default function EditTestResultStatus() {
  ...
  const [formData, setFormData] = useState<TestResultStatus>(() => createEmptyTestResultStatus());
  const [historyList, setHistoryList] = useState<TestResultStatusHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyTestResultStatus());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<TestResultStatus>(`/api/testresultstatus/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditTestResultStatus: error loading record", err);
      }

      try {
        const history = await api.get<TestResultStatusHistory[]>(`/api/testresultstatus/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditTestResultStatus: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditTestResultStatus: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<TestResultStatus>("/api/testresultstatus", formData);
      } else {
        await api.put<TestResultStatus>(`/api/testresultstatus/${id}`, formData);
      ...
      console.error("EditTestResultStatus: error saving testresultstatus", err);
    }
    navigate(returnUrl ?? "/testresultstatus");
  };
  ...
  // Soft delete -- DELETE /api/testresultstatus/{id} sets is_active=0 server-side
  // (see TestResultStatusLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Test Result Status? This cannot be undone.`)) return;
    try {
      await api.del(`/api/testresultstatus/${id}`);
    } catch (err) {
      console.error("EditTestResultStatus: error deleting testresultstatus", err);
      return;
    }
    navigate(returnUrl ?? "/testresultstatus");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Test Result Status</h3> : <h3>Edit Test Result Status {rwkString}</h3>}
