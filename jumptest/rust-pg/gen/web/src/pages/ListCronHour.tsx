...
// Row shape returned by GET /api/cronhour and /api/cronhour/view/{id}
// -- matches the CronHourView class generated for the dotnet/rust servers.
export interface CronHourView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "CronHour";
...
async function fetchList(api: ApiClient): Promise<CronHourView[]> {
  return api.get<CronHourView[]>("/api/cronhour");
}

export default function ListCronHour() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<CronHourView[] | null>(null);
  ...
  const listRef = useRef<CronHourView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListCronHour: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListCronHour: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<CronHourView>(`/api/cronhour/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListCronHour: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-cronhour?returnUrl=${returnUrl}`);
  }

  function onEdit(item: CronHourView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-cronhour/${item.id}?returnUrl=${returnUrl}`);
  }
