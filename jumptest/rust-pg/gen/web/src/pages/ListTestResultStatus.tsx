...
// Row shape returned by GET /api/testresultstatus and /api/testresultstatus/view/{id}
// -- matches the TestResultStatusView class generated for the dotnet/rust servers.
export interface TestResultStatusView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "TestResultStatus";
...
async function fetchList(api: ApiClient): Promise<TestResultStatusView[]> {
  return api.get<TestResultStatusView[]>("/api/testresultstatus");
}

export default function ListTestResultStatus() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<TestResultStatusView[] | null>(null);
  ...
  const listRef = useRef<TestResultStatusView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListTestResultStatus: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListTestResultStatus: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<TestResultStatusView>(`/api/testresultstatus/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListTestResultStatus: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-testresultstatus?returnUrl=${returnUrl}`);
  }

  function onEdit(item: TestResultStatusView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-testresultstatus/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Test Result Status</h1>
      <p>This page demonstrates fetching Test Result Status data from the server.</p>
