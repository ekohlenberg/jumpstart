...
// Raw shape sent/received by GET|POST|PUT /api/testresult[/{id}] -- matches
// the TestResult domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "TestResultHistory : TestResult").
export type TestResultHistory = TestResult;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Test Result ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "test_run_id", label: "Test Run", kind: "enum", fkVar: "testrun", isGlobal: false, isId: false },
  { key: "test_case_id", label: "Test Case", kind: "enum", fkVar: "testcase", isGlobal: false, isId: false },
  { key: "test_result_status_id", label: "Status", kind: "enum", fkVar: "testresultstatus", isGlobal: false, isId: false },
  { key: "executed_at", label: "Executed At", kind: "date", fkVar: "", isGlobal: false, isId: false },
  { key: "executed_by_id", label: "Executed By", kind: "enum", fkVar: "principal", isGlobal: false, isId: false },
  { key: "actual_result", label: "Actual Result", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyTestResult(): TestResult {
  ...
  return obj as unknown as TestResult;
}
...
export default function EditTestResult() {
  ...
  const [formData, setFormData] = useState<TestResult>(() => createEmptyTestResult());
  const [historyList, setHistoryList] = useState<TestResultHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyTestResult());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<TestResult>(`/api/testresult/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditTestResult: error loading record", err);
      }

      try {
        const history = await api.get<TestResultHistory[]>(`/api/testresult/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditTestResult: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditTestResult: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<TestResult>("/api/testresult", formData);
      } else {
        await api.put<TestResult>(`/api/testresult/${id}`, formData);
      ...
      console.error("EditTestResult: error saving testresult", err);
    }
    navigate(returnUrl ?? "/testresult");
  };
  ...
  // Soft delete -- DELETE /api/testresult/{id} sets is_active=0 server-side
  // (see TestResultLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/testresult/${id}`);
    } catch (err) {
      console.error("EditTestResult: error deleting testresult", err);
      return;
    }
    navigate(returnUrl ?? "/testresult");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Test Result</h3> : <h3>Edit Test Result {rwkString}</h3>}
