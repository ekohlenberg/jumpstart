...
// Raw shape sent/received by GET|POST|PUT /api/crondom[/{id}] -- matches
// the CronDom domain class generated for the dotnet/rust servers (every
// attribute, unlike the *View interface the list page works with).
export interface CronDom {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
// shared/dotnet/domain/template.generated.cs.cshtml's "CronDomHistory : CronDom").
export type CronDomHistory = CronDom;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Day Of Month ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Day Of Month", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
const OWN_COLUMNS: DataTableColumn[] = [
  { key: "id", label: "Day Of Month ID" },
  { key: "name", label: "Day Of Month" },
...
function createEmptyCronDom(): CronDom {
  ...
  return obj as unknown as CronDom;
}
...
export default function EditCronDom() {
  ...
  const [formData, setFormData] = useState<CronDom>(() => createEmptyCronDom());
  const [historyList, setHistoryList] = useState<CronDomHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyCronDom());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<CronDom>(`/api/crondom/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditCronDom: error loading record", err);
      }

      try {
        const history = await api.get<CronDomHistory[]>(`/api/crondom/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditCronDom: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditCronDom: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<CronDom>("/api/crondom", formData);
      } else {
        await api.put<CronDom>(`/api/crondom/${id}`, formData);
      ...
      console.error("EditCronDom: error saving crondom", err);
    }
    navigate(returnUrl ?? "/crondom");
  };
  ...
  // Soft delete -- DELETE /api/crondom/{id} sets is_active=0 server-side
  // (see CronDomLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/crondom/${id}`);
    } catch (err) {
      console.error("EditCronDom: error deleting crondom", err);
      return;
    }
    navigate(returnUrl ?? "/crondom");
  }
