...
// Raw shape sent/received by GET|POST|PUT /api/process[/{id}] -- matches
// the Process domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "ProcessHistory : Process").
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Process ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Name", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "script_id", label: "Script", kind: "number", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyProcess(): Process {
  ...
  return obj as unknown as Process;
}
...
export default function EditProcess() {
  ...
  const [formData, setFormData] = useState<Process>(() => createEmptyProcess());
  const [historyList, setHistoryList] = useState<ProcessHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyProcess());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<Process>(`/api/process/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditProcess: error loading record", err);
      }

      try {
        const history = await api.get<ProcessHistory[]>(`/api/process/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditProcess: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditProcess: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<Process>("/api/process", formData);
      } else {
        await api.put<Process>(`/api/process/${id}`, formData);
      ...
      console.error("EditProcess: error saving process", err);
    }
    navigate(returnUrl ?? "/process");
  };
  ...
  // Soft delete -- DELETE /api/process/{id} sets is_active=0 server-side
  // (see ProcessLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/process/${id}`);
    } catch (err) {
      console.error("EditProcess: error deleting process", err);
      return;
    }
    navigate(returnUrl ?? "/process");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Process</h3> : <h3>Edit Process {rwkString}</h3>}
