...
// Raw shape sent/received by GET|POST|PUT /api/cronminute[/{id}] -- matches
// the CronMinute domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "CronMinuteHistory : CronMinute").
export type CronMinuteHistory = CronMinute;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Minute ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Minute", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
const OWN_COLUMNS: DataTableColumn[] = [
  { key: "id", label: "Minute ID" },
  { key: "name", label: "Minute" },
...
export default function EditCronMinute() {
  ...
  const [formData, setFormData] = useState<CronMinute>(() => createEmptyCronMinute());
  const [historyList, setHistoryList] = useState<CronMinuteHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyCronMinute());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<CronMinute>(`/api/cronminute/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditCronMinute: error loading record", err);
      }

      try {
        const history = await api.get<CronMinuteHistory[]>(`/api/cronminute/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditCronMinute: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditCronMinute: error loading enum options", err));
  ...
  // Soft delete -- DELETE /api/cronminute/{id} sets is_active=0 server-side
  // (see CronMinuteLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    try {
      await api.del(`/api/cronminute/${id}`);
    } catch (err) {
      console.error("EditCronMinute: error deleting cronminute", err);
      return;
    }
    navigate(returnUrl ?? "/cronminute");
  }
