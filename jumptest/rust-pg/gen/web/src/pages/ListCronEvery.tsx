...
// Row shape returned by GET /api/cronevery and /api/cronevery/view/{id}
// -- matches the CronEveryView class generated for the dotnet/rust servers.
export interface CronEveryView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "CronEvery";
...
async function fetchList(api: ApiClient): Promise<CronEveryView[]> {
  return api.get<CronEveryView[]>("/api/cronevery");
}

export default function ListCronEvery() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<CronEveryView[] | null>(null);
  ...
  const listRef = useRef<CronEveryView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListCronEvery: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListCronEvery: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent<string>) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<CronEveryView>(`/api/cronevery/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListCronEvery: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-cronevery?returnUrl=${returnUrl}`);
  }

  function onEdit(item: CronEveryView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-cronevery/${item.id}?returnUrl=${returnUrl}`);
  }
