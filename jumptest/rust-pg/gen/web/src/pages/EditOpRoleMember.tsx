...
// Raw shape sent/received by GET|POST|PUT /api/oprolemember[/{id}] -- matches
// the OpRoleMember domain class generated for the dotnet/rust servers (every
// attribute, unlike the *View interface the list page works with).
export interface OpRoleMember {
  id: number;
  principal_id: number;
  op_role_id: number;
  is_active: number;
...
// shared/dotnet/domain/template.generated.cs.cshtml's "OpRoleMemberHistory : OpRoleMember").
export type OpRoleMemberHistory = OpRoleMember;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Member ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "principal_id", label: "Username", kind: "number", fkVar: "", isGlobal: false, isId: false },
  { key: "op_role_id", label: "Role", kind: "number", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyOpRoleMember(): OpRoleMember {
  ...
  return obj as unknown as OpRoleMember;
}
...
export default function EditOpRoleMember() {
  ...
  const [formData, setFormData] = useState<OpRoleMember>(() => createEmptyOpRoleMember());
  const [historyList, setHistoryList] = useState<OpRoleMemberHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyOpRoleMember());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<OpRoleMember>(`/api/oprolemember/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditOpRoleMember: error loading record", err);
      }

      try {
        const history = await api.get<OpRoleMemberHistory[]>(`/api/oprolemember/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditOpRoleMember: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditOpRoleMember: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<OpRoleMember>("/api/oprolemember", formData);
      } else {
        await api.put<OpRoleMember>(`/api/oprolemember/${id}`, formData);
      ...
      console.error("EditOpRoleMember: error saving oprolemember", err);
    }
    navigate(returnUrl ?? "/oprolemember");
  };
  ...
  // Soft delete -- DELETE /api/oprolemember/{id} sets is_active=0 server-side
  // (see OpRoleMemberLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Operation Role Members? This cannot be undone.`)) return;
    try {
      await api.del(`/api/oprolemember/${id}`);
    } catch (err) {
      console.error("EditOpRoleMember: error deleting oprolemember", err);
      return;
    }
    navigate(returnUrl ?? "/oprolemember");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Operation Role Members</h3> : <h3>Edit Operation Role Members {rwkString}</h3>}
