...
// Row shape returned by GET /api/execstatus and /api/execstatus/view/{id}
// -- matches the ExecStatusView class generated for the dotnet/rust servers.
export interface ExecStatusView {
  id: number;
  name: string;
  image: string;
  is_active: number;
...
const DOMAIN_OBJECT_NAME = "ExecStatus";
...
async function fetchList(api: ApiClient): Promise<ExecStatusView[]> {
  return api.get<ExecStatusView[]>("/api/execstatus");
}

export default function ListExecStatus() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<ExecStatusView[] | null>(null);
  ...
  const listRef = useRef<ExecStatusView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListExecStatus: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListExecStatus: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<ExecStatusView>(`/api/execstatus/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListExecStatus: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-execstatus?returnUrl=${returnUrl}`);
  }

  function onEdit(item: ExecStatusView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-execstatus/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Execution Status</h1>
      <p>This page demonstrates fetching Execution Status data from the server.</p>
