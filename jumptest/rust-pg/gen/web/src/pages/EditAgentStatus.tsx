...
// Raw shape sent/received by GET|POST|PUT /api/agentstatus[/{id}] -- matches
// the AgentStatus domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "AgentStatusHistory : AgentStatus").
export type AgentStatusHistory = AgentStatus;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Agent Status ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Status", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyAgentStatus(): AgentStatus {
  ...
  return obj as unknown as AgentStatus;
}
...
export default function EditAgentStatus() {
  ...
  const [formData, setFormData] = useState<AgentStatus>(() => createEmptyAgentStatus());
  const [historyList, setHistoryList] = useState<AgentStatusHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyAgentStatus());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<AgentStatus>(`/api/agentstatus/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditAgentStatus: error loading record", err);
      }

      try {
        const history = await api.get<AgentStatusHistory[]>(`/api/agentstatus/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditAgentStatus: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditAgentStatus: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<AgentStatus>("/api/agentstatus", formData);
      } else {
        await api.put<AgentStatus>(`/api/agentstatus/${id}`, formData);
      ...
      console.error("EditAgentStatus: error saving agentstatus", err);
    }
    navigate(returnUrl ?? "/agentstatus");
  };
  ...
  // Soft delete -- DELETE /api/agentstatus/{id} sets is_active=0 server-side
  // (see AgentStatusLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Agent Status? This cannot be undone.`)) return;
    try {
      await api.del(`/api/agentstatus/${id}`);
    } catch (err) {
      console.error("EditAgentStatus: error deleting agentstatus", err);
      return;
    }
    navigate(returnUrl ?? "/agentstatus");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Agent Status</h3> : <h3>Edit Agent Status {rwkString}</h3>}
