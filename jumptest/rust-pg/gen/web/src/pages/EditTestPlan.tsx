...
export default function EditTestPlan() {
  ...
  // Soft delete -- DELETE /api/testplan/{id} sets is_active=0 server-side
  // (see TestPlanLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Test Plan? This cannot be undone.`)) return;
    try {
      await api.del(`/api/testplan/${id}`);
    } catch (err) {
      console.error("EditTestPlan: error deleting testplan", err);
      return;
    }
    navigate(returnUrl ?? "/testplan");
  }
