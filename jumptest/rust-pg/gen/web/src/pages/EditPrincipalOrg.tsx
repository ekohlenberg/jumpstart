...
// Raw shape sent/received by GET|POST|PUT /api/principalorg[/{id}] -- matches
// the PrincipalOrg domain class generated for the dotnet/rust servers (every
// attribute, unlike the *View interface the list page works with).
export interface PrincipalOrg {
  id: number;
  org_id: number;
  principal_id: number;
  is_active: number;
...
// shared/dotnet/domain/template.generated.cs.cshtml's "PrincipalOrgHistory : PrincipalOrg").
export type PrincipalOrgHistory = PrincipalOrg;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Principal Org ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "org_id", label: "Organization ID", kind: "number", fkVar: "", isGlobal: false, isId: false },
  { key: "principal_id", label: "Principal ID", kind: "number", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyPrincipalOrg(): PrincipalOrg {
  ...
  return obj as unknown as PrincipalOrg;
}
...
export default function EditPrincipalOrg() {
  ...
  const [formData, setFormData] = useState<PrincipalOrg>(() => createEmptyPrincipalOrg());
  const [historyList, setHistoryList] = useState<PrincipalOrgHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyPrincipalOrg());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<PrincipalOrg>(`/api/principalorg/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditPrincipalOrg: error loading record", err);
      }

      try {
        const history = await api.get<PrincipalOrgHistory[]>(`/api/principalorg/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditPrincipalOrg: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditPrincipalOrg: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<PrincipalOrg>("/api/principalorg", formData);
      } else {
        await api.put<PrincipalOrg>(`/api/principalorg/${id}`, formData);
      ...
      console.error("EditPrincipalOrg: error saving principalorg", err);
    }
  ...
  return (
    <>
      {id == null ? <h3>Create Principals</h3> : <h3>Edit Principals {rwkString}</h3>}
