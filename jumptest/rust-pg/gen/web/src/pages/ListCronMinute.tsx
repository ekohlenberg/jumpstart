...
// Row shape returned by GET /api/cronminute and /api/cronminute/view/{id}
// -- matches the CronMinuteView class generated for the dotnet/rust servers.
export interface CronMinuteView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "CronMinute";
...
async function fetchList(api: ApiClient): Promise<CronMinuteView[]> {
  return api.get<CronMinuteView[]>("/api/cronminute");
}

export default function ListCronMinute() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<CronMinuteView[] | null>(null);
  ...
  const listRef = useRef<CronMinuteView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListCronMinute: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListCronMinute: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<CronMinuteView>(`/api/cronminute/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListCronMinute: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-cronminute?returnUrl=${returnUrl}`);
  }

  function onEdit(item: CronMinuteView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-cronminute/${item.id}?returnUrl=${returnUrl}`);
  }
