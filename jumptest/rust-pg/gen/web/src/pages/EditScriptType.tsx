...
// Raw shape sent/received by GET|POST|PUT /api/scripttype[/{id}] -- matches
// the ScriptType domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "ScriptTypeHistory : ScriptType").
export type ScriptTypeHistory = ScriptType;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Script Type ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
...
function createEmptyScriptType(): ScriptType {
  ...
  return obj as unknown as ScriptType;
}
...
export default function EditScriptType() {
  ...
  const [formData, setFormData] = useState<ScriptType>(() => createEmptyScriptType());
  const [historyList, setHistoryList] = useState<ScriptTypeHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyScriptType());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<ScriptType>(`/api/scripttype/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditScriptType: error loading record", err);
      }

      try {
        const history = await api.get<ScriptTypeHistory[]>(`/api/scripttype/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditScriptType: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditScriptType: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<ScriptType>("/api/scripttype", formData);
      } else {
        await api.put<ScriptType>(`/api/scripttype/${id}`, formData);
      ...
      console.error("EditScriptType: error saving scripttype", err);
    }
    navigate(returnUrl ?? "/scripttype");
  };
  ...
  // Soft delete -- DELETE /api/scripttype/{id} sets is_active=0 server-side
  // (see ScriptTypeLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Script Type? This cannot be undone.`)) return;
    try {
      await api.del(`/api/scripttype/${id}`);
    } catch (err) {
      console.error("EditScriptType: error deleting scripttype", err);
      return;
    }
    navigate(returnUrl ?? "/scripttype");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Script Type</h3> : <h3>Edit Script Type {rwkString}</h3>}
