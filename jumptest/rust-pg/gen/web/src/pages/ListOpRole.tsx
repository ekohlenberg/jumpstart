...
// Row shape returned by GET /api/oprole and /api/oprole/view/{id}
// -- matches the OpRoleView class generated for the dotnet/rust servers.
export interface OpRoleView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "OpRole";
...
async function fetchList(api: ApiClient): Promise<OpRoleView[]> {
  return api.get<OpRoleView[]>("/api/oprole");
}

export default function ListOpRole() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<OpRoleView[] | null>(null);
  ...
  const listRef = useRef<OpRoleView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListOpRole: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListOpRole: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<OpRoleView>(`/api/oprole/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListOpRole: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-oprole?returnUrl=${returnUrl}`);
  }

  function onEdit(item: OpRoleView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-oprole/${item.id}?returnUrl=${returnUrl}`);
  }
