...
// Row shape returned by GET /api/workflowtype and /api/workflowtype/view/{id}
// -- matches the WorkflowTypeView class generated for the dotnet/rust servers.
export interface WorkflowTypeView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "WorkflowType";
...
async function fetchList(api: ApiClient): Promise<WorkflowTypeView[]> {
  return api.get<WorkflowTypeView[]>("/api/workflowtype");
}

export default function ListWorkflowType() {
  ...
  const listRef = useRef<WorkflowTypeView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListWorkflowType: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListWorkflowType: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<WorkflowTypeView>(`/api/workflowtype/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListWorkflowType: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-workflowtype?returnUrl=${returnUrl}`);
  }

  function onEdit(item: WorkflowTypeView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-workflowtype/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Workflow Type</h1>
      <p>This page demonstrates fetching Workflow Type data from the server.</p>
