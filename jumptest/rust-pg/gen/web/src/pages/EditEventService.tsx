...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Event ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "event_type", label: "Event Type", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "objectname_filter", label: "Object Filter", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "methodname_filter", label: "Method Filter", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "script_id", label: "Script ID", kind: "enum", fkVar: "script", isGlobal: false, isId: false },
...
export default function EditEventService() {
  ...
  // Soft delete -- DELETE /api/eventservice/{id} sets is_active=0 server-side
  // (see EventServiceLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Events? This cannot be undone.`)) return;
    try {
      await api.del(`/api/eventservice/${id}`);
    } catch (err) {
      console.error("EditEventService: error deleting eventservice", err);
      return;
    }
    navigate(returnUrl ?? "/eventservice");
  }
