...
// Row shape returned by GET /api/org and /api/org/view/{id}
// -- matches the OrgView class generated for the dotnet/rust servers.
...
async function fetchList(api: ApiClient): Promise<OrgView[]> {
  return api.get<OrgView[]>("/api/org");
}
...
export default function ListOrg() {
  ...
  const [list, setList] = useState<OrgView[] | null>(null);
  ...
  const listRef = useRef<OrgView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListOrg: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListOrg: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent<string>) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<OrgView>(`/api/org/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListOrg: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-org?returnUrl=${returnUrl}`);
  }

  function onEdit(item: OrgView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-org/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Organization</h1>
      <p>This page demonstrates fetching Organization data from the server.</p>
