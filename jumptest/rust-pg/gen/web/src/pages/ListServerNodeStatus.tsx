...
// Row shape returned by GET /api/servernodestatus and /api/servernodestatus/view/{id}
// -- matches the ServerNodeStatusView class generated for the dotnet/rust servers.
export interface ServerNodeStatusView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "ServerNodeStatus";
...
async function fetchList(api: ApiClient): Promise<ServerNodeStatusView[]> {
  return api.get<ServerNodeStatusView[]>("/api/servernodestatus");
}

export default function ListServerNodeStatus() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<ServerNodeStatusView[] | null>(null);
  ...
  const listRef = useRef<ServerNodeStatusView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListServerNodeStatus: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListServerNodeStatus: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent<string>) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<ServerNodeStatusView>(`/api/servernodestatus/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListServerNodeStatus: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-servernodestatus?returnUrl=${returnUrl}`);
  }

  function onEdit(item: ServerNodeStatusView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-servernodestatus/${item.id}?returnUrl=${returnUrl}`);
  }
