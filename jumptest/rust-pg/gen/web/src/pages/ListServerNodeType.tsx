...
// Row shape returned by GET /api/servernodetype and /api/servernodetype/view/{id}
// -- matches the ServerNodeTypeView class generated for the dotnet/rust servers.
export interface ServerNodeTypeView {
  id: number;
  name: string;
  is_active: number;
  created_by: string;
...
const DOMAIN_OBJECT_NAME = "ServerNodeType";
...
async function fetchList(api: ApiClient): Promise<ServerNodeTypeView[]> {
  return api.get<ServerNodeTypeView[]>("/api/servernodetype");
}

export default function ListServerNodeType() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<ServerNodeTypeView[] | null>(null);
  ...
  const listRef = useRef<ServerNodeTypeView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListServerNodeType: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListServerNodeType: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent<string>) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<ServerNodeTypeView>(`/api/servernodetype/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListServerNodeType: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-servernodetype?returnUrl=${returnUrl}`);
  }

  function onEdit(item: ServerNodeTypeView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-servernodetype/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Server Node Type</h1>
      <p>This page demonstrates fetching Server Node Type data from the server.</p>
