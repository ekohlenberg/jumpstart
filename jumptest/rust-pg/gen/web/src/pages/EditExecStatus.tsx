...
// Raw shape sent/received by GET|POST|PUT /api/execstatus[/{id}] -- matches
// the ExecStatus domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "ExecStatusHistory : ExecStatus").
export type ExecStatusHistory = ExecStatus;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Exec Status ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Name", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "image", label: "Image", kind: "string", fkVar: "", isGlobal: false, isId: false },
...
function createEmptyExecStatus(): ExecStatus {
  ...
  return obj as unknown as ExecStatus;
}
...
export default function EditExecStatus() {
  ...
  const [formData, setFormData] = useState<ExecStatus>(() => createEmptyExecStatus());
  const [historyList, setHistoryList] = useState<ExecStatusHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyExecStatus());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<ExecStatus>(`/api/execstatus/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditExecStatus: error loading record", err);
      }

      try {
        const history = await api.get<ExecStatusHistory[]>(`/api/execstatus/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditExecStatus: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditExecStatus: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<ExecStatus>("/api/execstatus", formData);
      } else {
        await api.put<ExecStatus>(`/api/execstatus/${id}`, formData);
      ...
      console.error("EditExecStatus: error saving execstatus", err);
    }
    navigate(returnUrl ?? "/execstatus");
  };
  ...
  // Soft delete -- DELETE /api/execstatus/{id} sets is_active=0 server-side
  // (see ExecStatusLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Execution Status? This cannot be undone.`)) return;
    try {
      await api.del(`/api/execstatus/${id}`);
    } catch (err) {
      console.error("EditExecStatus: error deleting execstatus", err);
      return;
    }
    navigate(returnUrl ?? "/execstatus");
  }

  const rwkString = [formData.name, formData.image]
  ...
  return (
    <>
      {id == null ? <h3>Create Execution Status</h3> : <h3>Edit Execution Status {rwkString}</h3>}
