...
// Raw shape sent/received by GET|POST|PUT /api/script[/{id}] -- matches
// the Script domain class generated for the dotnet/rust servers (every
...
// shared/dotnet/domain/template.generated.cs.cshtml's "ScriptHistory : Script").
export type ScriptHistory = Script;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Script ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "name", label: "Name", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "source", label: "Source Code", kind: "string", fkVar: "", isGlobal: false, isId: false },
  { key: "script_type_id", label: "Script Type", kind: "enum", fkVar: "scripttype", isGlobal: false, isId: false },
...
function createEmptyScript(): Script {
  ...
  return obj as unknown as Script;
}
...
export default function EditScript() {
  ...
  const [formData, setFormData] = useState<Script>(() => createEmptyScript());
  const [historyList, setHistoryList] = useState<ScriptHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyScript());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<Script>(`/api/script/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditScript: error loading record", err);
      }

      try {
        const history = await api.get<ScriptHistory[]>(`/api/script/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditScript: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditScript: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<Script>("/api/script", formData);
      } else {
        await api.put<Script>(`/api/script/${id}`, formData);
      ...
      console.error("EditScript: error saving script", err);
    }
    navigate(returnUrl ?? "/script");
  };
  ...
  // Soft delete -- DELETE /api/script/{id} sets is_active=0 server-side
  // (see ScriptLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Scripts? This cannot be undone.`)) return;
    try {
      await api.del(`/api/script/${id}`);
    } catch (err) {
      console.error("EditScript: error deleting script", err);
      return;
    }
    navigate(returnUrl ?? "/script");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Scripts</h3> : <h3>Edit Scripts {rwkString}</h3>}
