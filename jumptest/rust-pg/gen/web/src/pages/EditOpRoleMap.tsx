...
// Raw shape sent/received by GET|POST|PUT /api/oprolemap[/{id}] -- matches
// the OpRoleMap domain class generated for the dotnet/rust servers (every
// attribute, unlike the *View interface the list page works with).
export interface OpRoleMap {
  id: number;
  op_id: number;
  op_role_id: number;
  is_active: number;
...
// shared/dotnet/domain/template.generated.cs.cshtml's "OpRoleMapHistory : OpRoleMap").
export type OpRoleMapHistory = OpRoleMap;
...
const FORM_FIELDS: FormField[] = [
  { key: "id", label: "Operation Role Map ID", kind: "number", fkVar: "", isGlobal: false, isId: true },
  { key: "op_id", label: "Operation ID", kind: "number", fkVar: "", isGlobal: false, isId: false },
  { key: "op_role_id", label: "Role ID", kind: "number", fkVar: "", isGlobal: false, isId: false },
...
const OWN_COLUMNS: DataTableColumn[] = [
  { key: "id", label: "Operation Role Map ID" },
...
function createEmptyOpRoleMap(): OpRoleMap {
  ...
  return obj as unknown as OpRoleMap;
}
...
export default function EditOpRoleMap() {
  ...
  const [formData, setFormData] = useState<OpRoleMap>(() => createEmptyOpRoleMap());
  const [historyList, setHistoryList] = useState<OpRoleMapHistory[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      if (id == null) {
        setFormData(createEmptyOpRoleMap());
        setHistoryList(null);
        return;
      }

      try {
        const record = await api.get<OpRoleMap>(`/api/oprolemap/${id}`);
        if (!cancelled) setFormData(record);
      } catch (err) {
        console.error("EditOpRoleMap: error loading record", err);
      }

      try {
        const history = await api.get<OpRoleMapHistory[]>(`/api/oprolemap/${id}/history`);
        if (!cancelled) setHistoryList(history);
      } catch (err) {
        console.error("EditOpRoleMap: error loading history", err);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .catch((err) => console.error("EditOpRoleMap: error loading enum options", err));
  ...
  const handleSubmit: FormEventHandler = async (e) => {
    ...
    try {
      if (id == null) {
        await api.post<OpRoleMap>("/api/oprolemap", formData);
      } else {
        await api.put<OpRoleMap>(`/api/oprolemap/${id}`, formData);
      ...
      console.error("EditOpRoleMap: error saving oprolemap", err);
    }
    navigate(returnUrl ?? "/oprolemap");
  };
  ...
  // Soft delete -- DELETE /api/oprolemap/{id} sets is_active=0 server-side
  // (see OpRoleMapLogic.delete() / server/dotnet/api/template.api.generated.cs.cshtml's
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete this Operation Group Map? This cannot be undone.`)) return;
    try {
      await api.del(`/api/oprolemap/${id}`);
    } catch (err) {
      console.error("EditOpRoleMap: error deleting oprolemap", err);
      return;
    }
    navigate(returnUrl ?? "/oprolemap");
  }
  ...
  return (
    <>
      {id == null ? <h3>Create Operation Group Map</h3> : <h3>Edit Operation Group Map {rwkString}</h3>}
