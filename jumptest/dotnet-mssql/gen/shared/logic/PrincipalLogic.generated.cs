...
namespace jumptest
{
    ...
    public interface IPrincipalLogic 
    {
        ...
        void delete( long id );
        PrincipalView view(long id);
        
    
    }

    public partial class PrincipalLogic : BaseLogic, IPrincipalLogic
    {

        /// <summary>
        /// Creates and initializes a proxy instance of the logic implementation for the domain object.
        /// </summary>
        /// <returns>An initialized proxy implementing <see cref="IPrincipalLogic"/>.</returns>
        ...
        /// </summary>
        /// <returns>A direct <see cref="PrincipalLogic"/> instance implementing <see cref="IPrincipalLogic"/>.</returns>
        public static IPrincipalLogic Unsafe()
        {
            return new PrincipalLogic();
        }

        /// <summary>
        /// Selects all domain objects using the default return type of the domain object.
        /// </summary>
        /// <returns>List of <see cref="Principal"/> instances.</returns>
        public virtual List<Principal> select()
        {
        ...
        {
            Logger.Debug("Processing PrincipalLogic select<TBaseObject> List");

            List<TBaseObject> principals = select<TBaseObject>("core.principal-select");
        ...
        {
            Logger.Debug($"Processing PrincipalLogic select<TBaseObject> with query: {queryName}");
        ...
        /// <returns>List of <see cref="PrincipalHistory"/> entries associated with the given id.</returns>
        public virtual List<PrincipalHistory> history(long id)
        {
            ...
            List<PrincipalHistory> principalHistory = DBPersist.select<PrincipalHistory>(
                $"SELECT * FROM core.principal WHERE id = {id} ORDER BY txn_id DESC");
        ...
        {
            Logger.Debug($"Processing PrincipalLogic children<TBaseObject> for ID={id}, childSuffix={childSuffix}");

            string queryName = "core.principal-children-" + childSuffix;
        ...
        /// for how the "core.principal-map-{mapSuffix}" query is generated.
        /// </summary>
        ...
        {
            Logger.Debug($"Processing PrincipalLogic maplist for ID={id}, mapSuffix={mapSuffix}");

            string queryName = "core.principal-map-" + mapSuffix;
        ...
        /// <returns>An instance of <see cref="Principal"/> if found; otherwise <c>null</c>.</returns>
        public virtual Principal get(long id)
        {
            Logger.Debug($"Processing PrincipalLogic get ID={id}");

            Principal principal = DBPersist.select<Principal>($"SELECT * FROM core.principal WHERE id = {id} AND is_active = 1").FirstOrDefault();
        ...
        public virtual PrincipalView view(long id) 
        {
            Logger.Debug($"Processing PrincipalLogic view<TBaseObject> ID={id}");

            string queryName = "core.principal-get";
            List<PrincipalView> PrincipalView = DBPersist.ExecuteQueryByName<PrincipalView>(queryName, new BaseObject() { { "id", id } });
            

            return PrincipalView.FirstOrDefault();
        }
        /// <summary>
        /// Inserts a new domain object into persistence.
        /// </summary>
        /// <param name="principal">The domain object instance to insert.</param>
        public virtual void insert(Principal principal)
        {
            Logger.Debug($"Processing PrincipalLogic insert: {principal}");
        ...
        /// </summary>
        /// <param name="principal">The domain object instance to put.</param>
        public virtual void put(Principal principal)
        {
            Logger.Debug($"Processing PrincipalLogic put: {principal}");
        ...
        /// <param name="principal">The updated domain object instance.</param>
        public virtual void update(long id, Principal principal)
        {
            Logger.Debug($"Processing PrincipalLogic update: ID = {id}\n{principal}");
