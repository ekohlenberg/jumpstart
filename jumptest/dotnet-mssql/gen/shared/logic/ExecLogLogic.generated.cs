...
namespace jumptest
{
    ...
    public interface IExecLogLogic 
    {
        // Generated methods
        List<ExecLog> select();
        ...
        List<ExecLogHistory> history(long id);
        void insert(ExecLog execlog);
        void update(long id, ExecLog execlog);
        void put(ExecLog execlog);
        void delete( long id );
        ExecLogView view(long id);
        
    
    }

    public partial class ExecLogLogic : BaseLogic, IExecLogLogic
    {

        /// <summary>
        /// Creates and initializes a proxy instance of the logic implementation for the domain object.
        /// </summary>
        /// <returns>An initialized proxy implementing <see cref="IExecLogLogic"/>.</returns>
        public static IExecLogLogic Create()
        {
            var execlog = new ExecLogLogic();

            var proxy = DispatchProxy.Create<IExecLogLogic, Proxy<IExecLogLogic>>();
            ((Proxy<IExecLogLogic>)proxy).Initialize();
            ((Proxy<IExecLogLogic>)proxy).Target = execlog;
            ((Proxy<IExecLogLogic>)proxy).DomainObj = "ExecLog";
        ...
        /// </summary>
        /// <returns>A direct <see cref="ExecLogLogic"/> instance implementing <see cref="IExecLogLogic"/>.</returns>
        public static IExecLogLogic Unsafe()
        {
            return new ExecLogLogic();
        }

        /// <summary>
        /// Selects all domain objects using the default return type of the domain object.
        /// </summary>
        /// <returns>List of <see cref="ExecLog"/> instances.</returns>
        public virtual List<ExecLog> select()
        {
            return select<ExecLog>();
        }
        ...
        {
            Logger.Debug("Processing ExecLogLogic select<TBaseObject> List");

            List<TBaseObject> execlogs = select<TBaseObject>("core.exec_log-select");
        ...
        {
            Logger.Debug($"Processing ExecLogLogic select<TBaseObject> with query: {queryName}");

            List<TBaseObject> execlogs = DBPersist.ExecuteQueryByName<TBaseObject>(queryName);
        ...
        /// <returns>List of <see cref="ExecLogHistory"/> entries associated with the given id.</returns>
        public virtual List<ExecLogHistory> history(long id)
        {
            ...
            List<ExecLogHistory> execlogHistory = DBPersist.select<ExecLogHistory>(
                $"SELECT * FROM core.exec_log WHERE id = {id} ORDER BY txn_id DESC");

            return execlogHistory;
        }
        ...
        {
            Logger.Debug($"Processing ExecLogLogic children<TBaseObject> for ID={id}, childSuffix={childSuffix}");

            string queryName = "core.exec_log-children-" + childSuffix;
        ...
        /// for how the "core.exec_log-map-{mapSuffix}" query is generated.
        /// </summary>
        ...
        {
            Logger.Debug($"Processing ExecLogLogic maplist for ID={id}, mapSuffix={mapSuffix}");

            string queryName = "core.exec_log-map-" + mapSuffix;
        ...
        /// <returns>An instance of <see cref="ExecLog"/> if found; otherwise <c>null</c>.</returns>
        public virtual ExecLog get(long id)
        {
            Logger.Debug($"Processing ExecLogLogic get ID={id}");

            ExecLog execlog = DBPersist.select<ExecLog>($"SELECT * FROM core.exec_log WHERE id = {id} AND is_active = 1").FirstOrDefault();
        ...
        public virtual ExecLogView view(long id) 
        {
            Logger.Debug($"Processing ExecLogLogic view<TBaseObject> ID={id}");

            string queryName = "core.exec_log-get";
            List<ExecLogView> ExecLogView = DBPersist.ExecuteQueryByName<ExecLogView>(queryName, new BaseObject() { { "id", id } });
            

            return ExecLogView.FirstOrDefault();
        }
        /// <summary>
        /// Inserts a new domain object into persistence.
        /// </summary>
        /// <param name="execlog">The domain object instance to insert.</param>
        public virtual void insert(ExecLog execlog)
        {
            Logger.Debug($"Processing ExecLogLogic insert: {execlog}");
        ...
        /// </summary>
        /// <param name="execlog">The domain object instance to put.</param>
        public virtual void put(ExecLog execlog)
        {
            Logger.Debug($"Processing ExecLogLogic put: {execlog}");

            execlog.is_active = 1;

            DBPersist.put(execlog);
        }
        ...
        /// <param name="execlog">The updated domain object instance.</param>
        public virtual void update(long id, ExecLog execlog)
        {
            Logger.Debug($"Processing ExecLogLogic update: ID = {id}\n{execlog}");
