...
namespace jumptest
{
    ...
    public interface IWorkflowLogic 
    {
        ...
        void delete( long id );
        WorkflowView view(long id);
        
    
    }

    public partial class WorkflowLogic : BaseLogic, IWorkflowLogic
    {

        /// <summary>
        /// Creates and initializes a proxy instance of the logic implementation for the domain object.
        /// </summary>
        /// <returns>An initialized proxy implementing <see cref="IWorkflowLogic"/>.</returns>
        ...
        /// </summary>
        /// <returns>A direct <see cref="WorkflowLogic"/> instance implementing <see cref="IWorkflowLogic"/>.</returns>
        public static IWorkflowLogic Unsafe()
        {
        ...
        /// </summary>
        /// <returns>List of <see cref="Workflow"/> instances.</returns>
        public virtual List<Workflow> select()
        {
        ...
        {
            Logger.Debug("Processing WorkflowLogic select<TBaseObject> List");
        ...
        {
            Logger.Debug($"Processing WorkflowLogic select<TBaseObject> with query: {queryName}");
        ...
        /// <returns>List of <see cref="WorkflowHistory"/> entries associated with the given id.</returns>
        public virtual List<WorkflowHistory> history(long id)
        {
            ...
            List<WorkflowHistory> workflowHistory = DBPersist.select<WorkflowHistory>(
                $"SELECT * FROM core.workflow WHERE id = {id} ORDER BY txn_id DESC");
        ...
        {
            Logger.Debug($"Processing WorkflowLogic children<TBaseObject> for ID={id}, childSuffix={childSuffix}");
        ...
        /// for how the "core.workflow-map-{mapSuffix}" query is generated.
        /// </summary>
        ...
        {
            Logger.Debug($"Processing WorkflowLogic maplist for ID={id}, mapSuffix={mapSuffix}");

            string queryName = "core.workflow-map-" + mapSuffix;
        ...
        /// <returns>An instance of <see cref="Workflow"/> if found; otherwise <c>null</c>.</returns>
        public virtual Workflow get(long id)
        {
            Logger.Debug($"Processing WorkflowLogic get ID={id}");

            Workflow workflow = DBPersist.select<Workflow>($"SELECT * FROM core.workflow WHERE id = {id} AND is_active = 1").FirstOrDefault();
        ...
        public virtual WorkflowView view(long id) 
        {
            Logger.Debug($"Processing WorkflowLogic view<TBaseObject> ID={id}");

            string queryName = "core.workflow-get";
            List<WorkflowView> WorkflowView = DBPersist.ExecuteQueryByName<WorkflowView>(queryName, new BaseObject() { { "id", id } });
            

            return WorkflowView.FirstOrDefault();
        }
        /// <summary>
        /// Inserts a new domain object into persistence.
        /// </summary>
        /// <param name="workflow">The domain object instance to insert.</param>
        public virtual void insert(Workflow workflow)
        {
            Logger.Debug($"Processing WorkflowLogic insert: {workflow}");
        ...
        /// </summary>
        /// <param name="workflow">The domain object instance to put.</param>
        public virtual void put(Workflow workflow)
        {
            Logger.Debug($"Processing WorkflowLogic put: {workflow}");
        ...
        /// <param name="workflow">The updated domain object instance.</param>
        public virtual void update(long id, Workflow workflow)
        {
            Logger.Debug($"Processing WorkflowLogic update: ID = {id}\n{workflow}");
