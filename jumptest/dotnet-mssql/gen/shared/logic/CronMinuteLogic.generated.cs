...
namespace jumptest
{
    ...
    public interface ICronMinuteLogic 
    {
        // Generated methods
        List<CronMinute> select();
        List<TBaseObject> select<TBaseObject>() where TBaseObject : BaseObject, new();
        List<TBaseObject> select<TBaseObject>(string queryName) where TBaseObject : BaseObject, new();
        CronMinute get(long id);
       

        List<TBaseObject> children<TBaseObject>(long id, string childSuffix) where TBaseObject : BaseObject, new();
        List<MapOption> maplist(long id, string mapSuffix);
        List<CronMinuteHistory> history(long id);
        void insert(CronMinute cronminute);
        void update(long id, CronMinute cronminute);
        void put(CronMinute cronminute);
        void delete( long id );
        CronMinuteView view(long id);
        
    
    }

    public partial class CronMinuteLogic : BaseLogic, ICronMinuteLogic
    {

        /// <summary>
        /// Creates and initializes a proxy instance of the logic implementation for the domain object.
        /// </summary>
        /// <returns>An initialized proxy implementing <see cref="ICronMinuteLogic"/>.</returns>
        public static ICronMinuteLogic Create()
        {
            var cronminute = new CronMinuteLogic();

            var proxy = DispatchProxy.Create<ICronMinuteLogic, Proxy<ICronMinuteLogic>>();
            ((Proxy<ICronMinuteLogic>)proxy).Initialize();
            ((Proxy<ICronMinuteLogic>)proxy).Target = cronminute;
            ((Proxy<ICronMinuteLogic>)proxy).DomainObj = "CronMinute";
        ...
        /// </summary>
        /// <returns>A direct <see cref="CronMinuteLogic"/> instance implementing <see cref="ICronMinuteLogic"/>.</returns>
        public static ICronMinuteLogic Unsafe()
        {
            return new CronMinuteLogic();
        }

        /// <summary>
        /// Selects all domain objects using the default return type of the domain object.
        /// </summary>
        /// <returns>List of <see cref="CronMinute"/> instances.</returns>
        public virtual List<CronMinute> select()
        {
            return select<CronMinute>();
        }
        ...
        {
            Logger.Debug("Processing CronMinuteLogic select<TBaseObject> List");

            List<TBaseObject> cronminutes = select<TBaseObject>("core.cron_minute-select");
        ...
        {
            Logger.Debug($"Processing CronMinuteLogic select<TBaseObject> with query: {queryName}");

            List<TBaseObject> cronminutes = DBPersist.ExecuteQueryByName<TBaseObject>(queryName);
        ...
        /// <returns>List of <see cref="CronMinuteHistory"/> entries associated with the given id.</returns>
        public virtual List<CronMinuteHistory> history(long id)
        {
            ...
            List<CronMinuteHistory> cronminuteHistory = DBPersist.select<CronMinuteHistory>(
                $"SELECT * FROM core.cron_minute WHERE id = {id} ORDER BY txn_id DESC");

            return cronminuteHistory;
        }
        ...
        {
            Logger.Debug($"Processing CronMinuteLogic children<TBaseObject> for ID={id}, childSuffix={childSuffix}");

            string queryName = "core.cron_minute-children-" + childSuffix;
        ...
        /// for how the "core.cron_minute-map-{mapSuffix}" query is generated.
        /// </summary>
        ...
        {
            Logger.Debug($"Processing CronMinuteLogic maplist for ID={id}, mapSuffix={mapSuffix}");

            string queryName = "core.cron_minute-map-" + mapSuffix;
        ...
        /// <returns>An instance of <see cref="CronMinute"/> if found; otherwise <c>null</c>.</returns>
        public virtual CronMinute get(long id)
        {
            Logger.Debug($"Processing CronMinuteLogic get ID={id}");

            CronMinute cronminute = DBPersist.select<CronMinute>($"SELECT * FROM core.cron_minute WHERE id = {id} AND is_active = 1").FirstOrDefault();
        ...
        public virtual CronMinuteView view(long id) 
        {
            Logger.Debug($"Processing CronMinuteLogic view<TBaseObject> ID={id}");

            string queryName = "core.cron_minute-get";
            List<CronMinuteView> CronMinuteView = DBPersist.ExecuteQueryByName<CronMinuteView>(queryName, new BaseObject() { { "id", id } });
            

            return CronMinuteView.FirstOrDefault();
        }
        /// <summary>
        /// Inserts a new domain object into persistence.
        /// </summary>
        /// <param name="cronminute">The domain object instance to insert.</param>
        public virtual void insert(CronMinute cronminute)
        {
            Logger.Debug($"Processing CronMinuteLogic insert: {cronminute}");

            cronminute.is_active = 1;

            DBPersist.insert(cronminute);
        }

        /// <summary>
        /// Inserts or updates (put) the domain object in persistence.
        /// </summary>
        /// <param name="cronminute">The domain object instance to put.</param>
        public virtual void put(CronMinute cronminute)
        {
            Logger.Debug($"Processing CronMinuteLogic put: {cronminute}");

            cronminute.is_active = 1;

            DBPersist.put(cronminute);
        }
        ...
        /// <param name="cronminute">The updated domain object instance.</param>
        public virtual void update(long id, CronMinute cronminute)
        {
            Logger.Debug($"Processing CronMinuteLogic update: ID = {id}\n{cronminute}");
        ...
        {
            CronMinute cronminute = get(id);
            cronminute.is_active = 0;
            DBPersist.update(cronminute);
        }
