...
namespace jumptest
{
    ...
    public interface ISqlLogic 
    {
        ...
        void put(Sql sql);
        void delete( long id );
        SqlView view(long id);
        
    
    }

    public partial class SqlLogic : BaseLogic, ISqlLogic
    {

        /// <summary>
        /// Creates and initializes a proxy instance of the logic implementation for the domain object.
        /// </summary>
        /// <returns>An initialized proxy implementing <see cref="ISqlLogic"/>.</returns>
        ...
        /// </summary>
        /// <returns>A direct <see cref="SqlLogic"/> instance implementing <see cref="ISqlLogic"/>.</returns>
        public static ISqlLogic Unsafe()
        {
            return new SqlLogic();
        }
        ...
        /// </summary>
        /// <returns>List of <see cref="Sql"/> instances.</returns>
        public virtual List<Sql> select()
        {
            return select<Sql>();
        }
        ...
        /// <returns>List of <see cref="SqlHistory"/> entries associated with the given id.</returns>
        public virtual List<SqlHistory> history(long id)
        {
            ...
            List<SqlHistory> sqlHistory = DBPersist.select<SqlHistory>(
                $"SELECT * FROM core.sql WHERE id = {id} ORDER BY txn_id DESC");
        ...
        /// for how the "core.sql-map-{mapSuffix}" query is generated.
        /// </summary>
        ...
        {
            Logger.Debug($"Processing SqlLogic maplist for ID={id}, mapSuffix={mapSuffix}");

            string queryName = "core.sql-map-" + mapSuffix;
        ...
        /// <returns>An instance of <see cref="Sql"/> if found; otherwise <c>null</c>.</returns>
        public virtual Sql get(long id)
        {
            Logger.Debug($"Processing SqlLogic get ID={id}");

            Sql sql = DBPersist.select<Sql>($"SELECT * FROM core.sql WHERE id = {id} AND is_active = 1").FirstOrDefault();
        ...
        public virtual SqlView view(long id) 
        {
            Logger.Debug($"Processing SqlLogic view<TBaseObject> ID={id}");

            string queryName = "core.sql-get";
            List<SqlView> SqlView = DBPersist.ExecuteQueryByName<SqlView>(queryName, new BaseObject() { { "id", id } });
            

            return SqlView.FirstOrDefault();
        }
        /// <summary>
        /// Inserts a new domain object into persistence.
        /// </summary>
        /// <param name="sql">The domain object instance to insert.</param>
        public virtual void insert(Sql sql)
        {
        ...
        /// </summary>
        /// <param name="sql">The domain object instance to put.</param>
        public virtual void put(Sql sql)
        {
        ...
        /// <param name="sql">The updated domain object instance.</param>
        public virtual void update(long id, Sql sql)
        {
