...
namespace jumptest
{
    ...
    {
        public Script(BaseObject baseObject) : base(baseObject)
        {
            Initialize();
        }
        ...
        protected override void Initialize()
        {
            ...
            auditTableName = "core.script"; // history rows live in the same table (is_active=0)
            isAudited = true;
            ...
            _defaults["source"] = default(string);
            ...
            public string source
            {
                get
                {
                    ...
                    try
                    {
                        ...
                        {
                        _source = Convert.ToString(this["source"].ToString());
                        }
                    }
                    catch(Exception )
                    {
                        //Logger.Error($"Error getting source: {e.Message}");
            ...
            [ColumnInfo("Script Type", "ScriptType", "enum", "script_type", "scripttype", false)]
            public long script_type_id
            {
                get
                {
                    long _script_type_id;
    ...
    public partial class ScriptHistory : Script
    {
        protected override void Initialize()
        {
    ...
    public class ScriptView : Script
    {

            [ColumnInfo("Script Type", "", "", "", "", false)]
            public string script_type_name
            {
                get
                {
                    string _script_type_name;

                            _script_type_name = string.Empty;
                                                
                    try
                    {
                        if(this.ContainsKey("script_type_name"))
                        {
                        _script_type_name = Convert.ToString(this["script_type_name"].ToString());
                        }
                    }
                    catch(Exception )
                    {
                        //Logger.Error($"Error getting script_type_name: {e.Message}");
                        _script_type_name = default(string);
                    }
                    return _script_type_name;
                }
                set
                {
                   
                    this["script_type_name"] = value;
                }
            }
                }
        }
