...
{
    /// <summary>
    /// Exports all rows from core.server_node_status to a CSV file.
    /// </summary>
    public static class ServerNodeStatusExporter
    {
        public static int Export(string outputPath)
        {
            var records = ServerNodeStatusLogic.Create().select<ServerNodeStatus>();
