...
{
    /// <summary>
    /// Exports all rows from app.test_result_status to a CSV file.
    /// </summary>
    public static class TestResultStatusExporter
    {
        public static int Export(string outputPath)
        {
            var records = TestResultStatusLogic.Create().select<TestResultStatus>();
