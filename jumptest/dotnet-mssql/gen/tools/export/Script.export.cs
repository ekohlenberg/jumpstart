...
{
    /// <summary>
    /// Exports all rows from core.script to a CSV file.
    /// </summary>
    public static class ScriptExporter
    {
        public static int Export(string outputPath)
        {
            var records = ScriptLogic.Create().select<Script>();
            ...
            foreach (var record in records)
            {
                ...
                csv.WriteField(record.script_type_id);
