...
{
    /// <summary>
    /// Exports all rows from core.op_role_map to a CSV file.
    /// </summary>
    public static class OpRoleMapExporter
    {
        public static int Export(string outputPath)
        {
            var records = OpRoleMapLogic.Create().select<OpRoleMap>();
            ...
            csv.WriteField("id");
            csv.WriteField("op_id");
            csv.WriteField("op_role_id");
            ...
            foreach (var record in records)
            {
                ...
                csv.WriteField(record.op_role_id);
