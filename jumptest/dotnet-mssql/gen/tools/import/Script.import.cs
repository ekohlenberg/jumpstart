...
{
    /// <summary>
    /// Imports rows from a CSV file into core.script.
    ...
    public static class ScriptImporter
    {
        public static int Import(string inputPath)
        {
            ...
            while (csv.Read())
            {
                ...
                // script_type_id  (BIGINT → long)
                if (csv.HeaderRecord != null && Array.IndexOf(csv.HeaderRecord, "script_type_id") >= 0)
                    script.script_type_id = string.IsNullOrEmpty(csv.GetField("script_type_id")) ? default : Convert.ToInt64(csv.GetField("script_type_id"));
