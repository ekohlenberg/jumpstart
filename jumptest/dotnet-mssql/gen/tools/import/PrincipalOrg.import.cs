...
{
    /// <summary>
    /// Imports rows from a CSV file into core.principal_org.
    ...
    /// </summary>
    public static class PrincipalOrgImporter
    {
        public static int Import(string inputPath)
        {
            ...
            while (csv.Read())
            {
                ...
                // org_id  (BIGINT → long)
                if (csv.HeaderRecord != null && Array.IndexOf(csv.HeaderRecord, "org_id") >= 0)
                    principalorg.org_id = string.IsNullOrEmpty(csv.GetField("org_id")) ? default : Convert.ToInt64(csv.GetField("org_id"));

                // principal_id  (BIGINT → long)
                if (csv.HeaderRecord != null && Array.IndexOf(csv.HeaderRecord, "principal_id") >= 0)
                    principalorg.principal_id = string.IsNullOrEmpty(csv.GetField("principal_id")) ? default : Convert.ToInt64(csv.GetField("principal_id"));
